...
static cl::OptionCategory pathProfilerCategory{"path profiler options"};
...
static cl::opt<string> profile{"p",
                               cl::desc("Path to path profiling results"),
                               cl::value_desc("filename"),
                               cl::cat{pathProfilerCategory}};

static cl::opt<unsigned> numberOfPaths{
    "n",
...
interpretResults(Module& module, std::string filename) {
  ...
  pm.add(new LoopInfoWrapperPass{});
  pm.add(new pathprofiling::PathEncodingPass{});
  pm.add(new pathprofiling::ProfileDecodingPass{filename,
                                                "top-five-paths.csv",
                                                numberOfPaths});
  pm.run(module);
}
...
main(int argc, char** argv) {
  ...
  cl::HideUnrelatedOptions(pathProfilerCategory);
